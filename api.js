(() => {
  ...
  var T = (function () {
        ...
        switch (r.event) {
            ...
            switch (c) {
              ...
              case "managed":
                switch (g) {
                  ...
                  case "invisible":
                    d(
                      `Invalid value for parameter "size", expected "${"compact"}" or "${"normal"}", got "${g}"`
                    );
                  case "normal":
              ...
              default:
                d(
                  `Invalid value for parameter "mode", expected "${"non-interactive"}", "${"managed"}" or "${"invisible"}", got "${c}"`
                );
                return;
            }
            n.style.display = "";
          ...
          case "tokenExpired": {
            let n = r.token,
              c = s.cbExpired;
            c && c(n),
              s.params["refresh-expired"] === "auto" && !s.isResetting && h(a);
            break;
          }
          case "timeout": {
            D(a);
    ...
    function u() {
      ...
      do {
        ...
        let r = "abcdefghijklmnopqrstuvwxyz0123456789",
          a = r.length;
        for (let s = 0; s < 5; s++)
          i += r.charAt(Math.floor(Math.random() * a));
      } while (e.has(i));
      return i;
    }
    function f(i) {
    ...
    return {
      ...
      ready(i) {
        ...
        typeof i != "function" &&
          d(
            `turnstile.ready() expected a "function" argument, got "${typeof onloadCb}"`
          ),
          (F = i);
      },
      render(i, r) {
        ...
        if (typeof i == "string")
          try {
            ...
            d(
              `Invalid type for  "container", expected "selector" or an implementation of "HTMLElement", got "${i}"`
            );
            return;
          }
        ...
        let s = Ie(a),
          ...
          { action: n, cData: c } = t,
          g = t.sitekey;
        (t.theme = (z = t.theme) != null ? z : "auto"),
          (t.retry = (U = t.retry) != null ? U : "auto"),
          (t["retry-interval"] = (V = t["retry-interval"]) != null ? V : ge),
          (t.size = (B = t.size) != null ? B : "normal");
        let v = t.callback,
        ...
        if (typeof g != "string") {
          d(
            `Invalid or missing type for parameter "sitekey", expected "string", got "${typeof g}"`
          );
          return;
        }
        if (!X(g)) {
        ...
        if (!P(t["retry-interval"])) {
          p(
            `Invalid data-retry-interval value: "${t["retry-interval"]}, expected an integer value > 0 and < 900000"`
          );
          return;
        }
        ...
        if (W($)) t["refresh-expired"] = $;
          ...
          d(
            `Invalid type for parameter "refressh-expired", expected never|manual|auto, got "${$}" ${typeof $}`
          );
          return;
        }
        ...
        if (n !== void 0 && typeof n != "string") {
          d(
            `Invalid type for parameter "action", expected "string", got "${typeof n}"`
          );
          return;
        }
        ...
        if (c !== void 0 && typeof c != "string") {
          d(
            `Invalid type for parameter "cData", expected "string", got "${typeof c}"`
          );
          return;
        }
        if (!K(c)) {
        ...
        if (
          (y.setAttribute(
            "src",
            `${ne}/cdn-cgi/challenge-platform/${ie}turnstile/if/ov2/av0/${M}/${g}/${t.theme}/${t.size}`
          ),
          (y.id = b),
          (y.tabIndex = (G = t.tabindex) != null ? G : 0),
          (y.title = "Widget containing a Cloudflare security challenge"),
          a.appendChild(y),
          (q = t["response-field"]) != null ? q : !0)
        ) {
          let m = document.createElement("input");
          if (
            ((m.type = "hidden"),
            (m.name = (Y = t["response-field-name"]) != null ? Y : fe),
            (m.id = `${b}_response`),
            a.appendChild(m),
            typeof t["response-field-name"] != "string" && ye(g))
          ) {
            let C = document.createElement("input");
            (C.type = "hidden"),
              (C.name = ue),
              (C.id = `${b}_legacy_response`),
              a.appendChild(C);
          }
        }
        if (k) {
  ...
  function Ie(e) {
    var R;
    let o = { sitekey: e.getAttribute("data-sitekey") },
    ...
    s &&
      (P(s)
        ? (o["retry-interval"] = s)
        : p(
            `Invalid data-retry-interval value: "${s}, expected an integer value > 0 and < 900000"`
          ));
    ...
    t &&
      (W(t)
        ? (o["refresh-expired"] = t)
        : p(
            `Unknown data-refresh-expired value: "${t}, expected either: never, auto or manual.`
          ));
  ...
  if (w) {
    let e = w.get("compat");
    (e == null ? void 0 : e.toLowerCase()) === "recaptcha"
      ? window.grecaptcha
        ? p(
            "grecaptcha is already defined. The compatibility layer will not be enabled"
          )
        : (p("Compatibility layer enabled"), (k = !0), (window.grecaptcha = T))
      : e !== null && p(`Unknown value for api.js?compat: "${e}", ignoring`),
      w.forEach(function (o, u) {
        E(["onload", "compat", "render"], u) ||
          p(`Unknown parameter passed to api.js: "?${u}=...", ignoring`);
      });
    let l = w.get("onload");
    l &&
      setTimeout(function () {
  ...
  we
    ? p(
        "Turnstile already has been loaded. Is Turnstile imported multiple times?"
      )
    : ("Proxy" in window
        ? (window.cfChallengeAPI = new Proxy(T, {
            get(e, l) {
              return (
                p(
                  "window.cfChallengeAPI is deprecated, please move to window.turnstile"
                ),
                e[l]
              );
            },
